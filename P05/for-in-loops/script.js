...
// ここにコードを書きましょう
function sayHellos() {
    for(const aisatu in hellos) {
        console.log(hellos[aisatu]);
    }
}
...
// ここにコードを書きましょう。
function getFirstObjectValues(obj) {
    const array = [];
    let num = 1;
    for(const arr of obj) {
        if(num == 1) {
           for(const obj2 in arr) {
                array.push(arr[obj2]);
            } 
            num++;  
        } else {
